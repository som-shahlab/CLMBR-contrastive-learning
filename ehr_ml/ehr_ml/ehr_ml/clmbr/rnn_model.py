...
def get_window_pairs(tl, windows):
	""" For getting window pairs for OCP CL. """
	num_pairs = int(len(windows)/2)
	pairs = []
	labels = []
	for i in range(num_pairs):
		win_1_start = windows.iloc[i]['start_day_idx']
...
class PatientRNN_OCP(nn.Module):
	def __init__(self, config, info):
		super().__init__()
		self.config = config
		self.info = info
	...
	def forward(self, rnn_input, windows=None):
		...
		if self.recurrent:
			...
			return padded_output.contiguous(), labels
		else:
			padded_output, _ = nn.utils.rnn.pad_packed_sequence(
				packed_sequence, batch_first=True
			)
			...
			return self.model(padded_output), labels

	...
	for i in range(args.iterations):
		...
		with DataLoader(dataset, model.config['num_first'], is_val=True, batch_size=999999, seed=args.seed, device=args.device) as loader:
			for j, batch in enumerate(loader):
				...
				z1 = outputs['z1']
				z2 = outputs['z2']
				idx = torch.randperm(z1.shape[0])
				z1 = z1[idx].view(z1.size())
				idx = torch.randperm(z2.shape[0])
				z2 = z2[idx].view(z2.size())
...
def eval_model(args, val_dataset, test_dataset, clmbr_hp, cl_hp=None, pooler='BL'):
	...
	pd.DataFrame(val_align).reset_index(drop=True).to_csv(f'{results_save_path}/val_align_vals_neg.csv')
	test_align = eval_alignment(args, mw, test_data, clmbr_hp)
	pd.DataFrame(test_align).reset_index(drop=True).to_csv(f'{results_save_path}/test_align_vals_neg.csv')
	clmbr_model.eval()
	val_uniform = eval_uniform(args, mw, val_data, clmbr_hp)
	pd.DataFrame(val_uniform).reset_index(drop=True).to_csv(f'{results_save_path}/val_uniform_vals_neg.csv')
	test_uniform = eval_uniform(args, mw, test_data, clmbr_hp)
	pd.DataFrame(test_uniform).reset_index(drop=True).to_csv(f'{results_save_path}/test_uniform_vals_neg.csv')
	...
	ci_df = pd.concat([ci_df,df])
	print(ci_df)
	ci_df.reset_index(drop=True).to_csv(f'{results_save_path}/uni_align_eval_neg.csv')
